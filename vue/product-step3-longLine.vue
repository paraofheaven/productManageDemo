<template>
	...
	<header class="product-header" data-type="1">
	    <a class="header-left" v-link="{path:'/step2'}">
	...
	<div class="product-wrap">
		<div class="card-container">
			<div class="mui-card mui-card-no-line card-1" 
				v-for="routeManage in routeManageArray">
				<div class="mui-card-header">第{{$index+1}}天</div>
				<div class="mui-card-content">
					<div class="mui-card-content-inner">
						<ul class="mui-table-view">
							<li class="mui-table-view-cell mui-table-view-cell-inline"
								v-for="route in routeManage.wanfaArray">
								<div class="mui-table-view-cell-column" name="content">{{route.title}}</div>
								<div class="mui-table-view-cell-column" 
									@click="removeRoute($parent.$index,$index)">
									<div class="mui-btn-remove-icon"></div>
								</div>
							</li>
						</ul>
					</div>
				...
				<div class="mui-card-footer">
					<a class="mui-btn-add" v-link="{path:'/step3#list',query:{new:false,index:$index}}">添加项目</a>
					<a class="mui-btn-remove" 
						@click="removeRouteManage(routeManage)" 
						v-show="$index">删除一天</a>
				</div>
			</div>
		</div>
		<div>
			<a class="click-add" 
				v-link="{path:'/step3#list',query:{new:true}}">增加一天</a>
		</div>
	</div>
